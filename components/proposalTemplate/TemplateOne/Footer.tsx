...
export default function Footer({ proposalData, fontFamily = "Poppins" }: FooterProps) {
  ...
  const signatureType = data?.proposalSetting?.signatures?.signatureType;
  const isDrawSignature =
    signatureType === "Draw" ||
    (!data?.signatureImageUrl && Boolean(data?.signatureText));
  const fallbackFont = `var(--proposal-font-family, "${fontFamily}", var(--font-sans))`;
  const signatureFontFamily = data?.signatureStyle?.trim() || fallbackFont;
  const signatureFontClass =
    signatureFontFamily.includes("Dancing Script")
      ? dancingScript.className
      : signatureFontFamily.includes("Pacifico")
        ? pacifico.className
        : signatureFontFamily.includes("Great Vibes")
          ...
          <div className="mb-8 rounded-2xl border border-slate-300 bg-white p-6 md:p-7 text-slate-900">
            ...
            <div className="mt-4 flex flex-col gap-2 items-start">
              {data?.signatureImageUrl ? (
                ...
                <p
                  className={`${signatureFontClass} signature-draw-text text-3xl font-semibold leading-none`}
                  style={{
                    color: data?.signatureColor || "var(--proposal-signature-color, #2DC6F5)",
                    ["--proposal-signature-font-family" as string]: signatureFontFamily,
                    fontWeight: isDrawSignature ? 400 : 600,
                  }}
                >
