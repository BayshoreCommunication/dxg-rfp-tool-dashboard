...
export default function VendorFactsSection({ proposalId, proposalTitle, vendorName, vendorEmail, submissionId, versionId, onIntelligence }: {
  ...
  return <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm" aria-labelledby="vendor-intelligence-title">
    ...
    {result?.run.status === "succeeded" && <>
      {blocked && <UnreadableFileCard warnings={result.run.warnings} vendorName={vendorName} vendorEmail={vendorEmail} proposalId={proposalId} proposalTitle={proposalTitle}/>}
