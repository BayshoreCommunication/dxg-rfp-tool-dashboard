...
function ReviewControls({ targetType, review, saving, onReview }: {
  ...
  if (review) return <p className="mt-3 text-[11px] font-semibold text-slate-500">Latest review: <span className="text-slate-800">{label(review.decision)}</span>{review.note ? ` · ${review.note}` : ""}</p>;
...
export default function VendorFactsSection({ proposalId, submissionId, versionId }: { proposalId: string; submissionId: string; versionId: string }) {
  ...
  const review = async (targetType: "fact" | "mapping", targetId: string, decision: "accepted" | "rejected" | "corrected" | "escalated", correctedValue?: string) => {
    if (!result) return;
    ...
    const response = await reviewVendorIntelligenceAction(proposalId, submissionId, versionId, result.run.runId, {
      ...
      note: decision === "corrected" ? "Value corrected by a proposal reviewer." : "", correctedPayload: correctedValue ? { normalizedValue: correctedValue, typedValue: { kind: "string", text: correctedValue } } : null,
    }, crypto.randomUUID());
    if (cancelled.current) return;
  ...
  return <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm" aria-labelledby="vendor-intelligence-title">
    <div className="flex flex-wrap items-start justify-between gap-3"><div>
      ...
      <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">Maps this response to the approved RFP requirements and extracts typed, cited facts. It does not rank, shortlist, or select the vendor.</p>
    </div><button type="button" onClick={() => void start()} disabled={processing || loading} className="inline-flex min-h-9 items-center gap-2 rounded-xl bg-[#087f69] px-3.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={13} className={processing ? "animate-spin" : ""}/>{processing ? "Generating…" : result ? "Refresh intelligence" : "Generate intelligence"}</button></div>
    ...
    {result?.run.status === "succeeded" && <>
      ...
      <div className="mt-4 flex gap-2 border-b border-slate-200"><button type="button" onClick={() => setTab("mappings")} className={`border-b-2 px-3 py-2 text-xs font-bold ${tab === "mappings" ? "border-[#008ad2] text-[#0076b4]" : "border-transparent text-slate-500"}`}>Requirement mappings</button><button type="button" onClick={() => setTab("facts")} className={`border-b-2 px-3 py-2 text-xs font-bold ${tab === "facts" ? "border-[#008ad2] text-[#0076b4]" : "border-transparent text-slate-500"}`}>Typed facts</button></div>
      {tab === "mappings" ? <ul className="mt-3 space-y-3">{result.mappings.map((mapping) => { const key = `mapping:${mapping.mappingId}`; return <li key={mapping.mappingId} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-bold text-slate-800">{mapping.requirementTitle}</p><p className="mt-0.5 text-[10px] uppercase tracking-wide text-slate-400">{mapping.mandatory ? "Mandatory" : "Evaluated"} · {Math.round(mapping.confidence * 100)}% confidence</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${relationshipStyle[mapping.relationship] ?? relationshipStyle.none}`}>{label(mapping.relationship)}</span></div><EvidenceList evidence={mapping.evidence}/><ReviewControls targetType="mapping" review={latestReviews.get(key)} saving={savingTarget === key} onReview={(decision, value) => review("mapping", mapping.mappingId, decision, value)}/></li>; })}</ul>
      : <ul className="mt-3 space-y-3">{result.facts.map((fact) => { const key = `fact:${fact.factId}`; return <li key={fact.factId} className={`rounded-xl border p-4 ${fact.contradictionGroup ? "border-amber-300 bg-amber-50/30" : "border-slate-200"}`}><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-bold text-slate-800">{fact.statement}</p><p className="mt-1 text-xs font-semibold text-[#0076b4]">{fact.normalizedValue || "Unspecified value"}</p><p className="mt-1 text-[10px] uppercase tracking-wide text-slate-400">{label(fact.family)} · {label(fact.valueKind)} · {fact.explicitness} · {Math.round(fact.confidence * 100)}% confidence</p></div>{fact.contradictionGroup && <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold uppercase text-amber-800">Contradiction</span>}</div><EvidenceList evidence={fact.citations}/><ReviewControls targetType="fact" review={latestReviews.get(key)} saving={savingTarget === key} onReview={(decision, value) => review("fact", fact.factId, decision, value)}/></li>; })}</ul>}
    </>}
  </section>;
}
