...
export default function VendorComparisonPanel({ responses, proposalId, requirementsApproved = false, preparedResponseIds, comparisonReadyResponseIds, returnTo = "/vendor-responses" }: { responses: VendorResponseItem[]; proposalId: string; requirementsApproved?: boolean; preparedResponseIds?: string[]; comparisonReadyResponseIds?: string[]; returnTo?: string }) {
  ...
  const comparisonComplete = preparationComplete && missingEvaluationCount === 0;
  ...
  return <section className="mb-5 rounded-2xl border border-violet-200 bg-violet-50/70 p-4" aria-labelledby="comparison-progress-title">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 id="comparison-progress-title" className="flex items-center gap-2 text-sm font-extrabold text-violet-950"><BarChart3 size={17}/>Comparison orchestration</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-violet-800">Freeze selected vendor versions into one durable run. After evidence review and human scoring are complete, the result applies eligibility gates and produces an advisory ranking; it never records the final selection.</p></div>
      {!view && <button type="button" disabled={busy || loading || candidates.length < 2 || !requirementsApproved || !comparisonComplete} onClick={() => void start()} title={!requirementsApproved ? "Approve the requirement registry before starting a comparison" : !preparationComplete ? "Complete requirement mapping for every selected vendor" : missingEvaluationCount > 0 ? "Complete evidence review and evaluator scorecards for every selected vendor" : undefined} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-violet-700 px-4 text-xs font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-45"><RefreshCw size={14} className={busy ? "animate-spin" : ""}/>{busy ? "Starting…" : `Start comparison (${candidates.length})`}</button>}
    </div>
