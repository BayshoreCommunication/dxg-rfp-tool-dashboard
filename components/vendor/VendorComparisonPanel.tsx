...
export default function VendorComparisonPanel({ responses, proposalId, requirementsApproved = false, preparedResponseIds, comparisonReadyResponseIds }: { responses: VendorResponseItem[]; proposalId: string; requirementsApproved?: boolean; preparedResponseIds?: string[]; comparisonReadyResponseIds?: string[]; returnTo?: string }) {
  ...
  const candidates = useMemo(() => versionedResponses.filter((response) => response.responseFormat === "structured_v1" || response.documents.length > 0 || response.message.trim().length > 0), [versionedResponses]);
  ...
  return <section className="mb-5 rounded-2xl border border-violet-200 bg-violet-50/70 p-4" aria-labelledby="comparison-progress-title">
    ...
    {!view && !loading && excludedEmptyCount > 0 && <p className="mt-4 rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700">{excludedEmptyCount} vendor {excludedEmptyCount === 1 ? "response was" : "responses were"} left out because {excludedEmptyCount === 1 ? "it has" : "they have"} no structured answers, message, or attached document to review.</p>}
