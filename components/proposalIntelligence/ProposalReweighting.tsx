...
export default function ProposalReweighting({ workspace }: { workspace: ComparisonWorkspace }) {
  ...
  return (
    <section className={cn(intelligenceSurfaceClasses.card, "mt-5")} aria-labelledby="reweighting-title">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-wide text-brand-dark">Comparison controls</p><h2 id="reweighting-title" className="mt-2 text-2xl font-extrabold text-navy">Reweight criteria</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-gray">Explore a bounded what-if ranking from the frozen mean human scores. Eligibility gates remain enforced, and these changes do not overwrite the approved matrix or persisted recommendation.</p></div><IntelligenceStatusChip status={available ? "complete" : "unavailable"} label={available ? "Criterion inputs available" : "Criterion inputs unavailable"} /></header>
      {!available ? (
        <div className={cn(intelligenceSurfaceClasses.block, "mt-5 bg-gray-panel")}><h3 className="text-sm font-extrabold text-navy">This run cannot be reweighted accurately</h3><p className="mt-2 text-sm leading-6 text-gray">This historical snapshot stores only each vendor’s total score. A new comparison created after the criterion-input contract upgrade will retain frozen per-criterion means, rubric maxima, and original weights.</p></div>
      ) : (
