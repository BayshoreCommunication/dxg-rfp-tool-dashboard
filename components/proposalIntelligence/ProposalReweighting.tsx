...
import { criterionOriginLabel } from "@/lib/proposalIntelligence/scoreExplanation";
...
export default function ProposalReweighting({ workspace }: { workspace: ComparisonWorkspace }) {
  ...
  return (
    <section className={cn(intelligenceSurfaceClasses.card, "mt-5")} aria-labelledby="reweighting-title">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h2 id="reweighting-title" className="text-2xl font-extrabold text-navy">Try different priorities</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-gray">Drag a slider to see how the ranking would change if an area mattered more. Nothing here is saved or changes the real result.</p></div>{!available && <IntelligenceStatusChip status="unavailable" label="Not available for this run" />}</header>
      {!available ? (
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-2">
          ...
          <div className={intelligenceSurfaceClasses.block}><h3 className="text-sm font-extrabold text-navy">What-if ranking</h3><ol className="mt-3 space-y-2">{ranking.map((vendor, index) => <li key={vendor.participantId} className={cn(intelligenceSurfaceClasses.block, "bg-gray-panel p-3")}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-extrabold text-navy">{vendor.eligible ? `#${index + 1} ` : ""}{vendor.vendorLabel}</p><p className="mt-1 text-xs text-gray">{vendor.eligible ? "Meets all must-pass requirements" : "Excluded — missed a must-pass requirement"}</p></div><span className="font-mono text-lg font-extrabold text-brand-dark">{vendor.score.toFixed(2)}</span></div><details className="mt-2"><summary className="cursor-pointer text-xs font-bold text-brand-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">How this score adds up</summary><ul className="mt-2 space-y-1.5 text-xs leading-5 text-gray">{vendor.breakdown.map((item) => <li key={item.criterionId}><span className="font-bold text-navy">{item.name}</span> · scored <span className="tabular-nums">{item.meanScore.toFixed(2)} of {item.rubricMaximum.toFixed(0)}</span> at <span className="tabular-nums">{item.weight.toFixed(1)}%</span> weight → <span className="tabular-nums font-bold text-navy">{item.contribution.toFixed(2)} points</span>{item.origin !== "unknown" && <span className="block text-[11px]">{criterionOriginLabel[item.origin]}</span>}</li>)}</ul></details></li>)}</ol></div>
        </div>
      )}
    </section>
  );
