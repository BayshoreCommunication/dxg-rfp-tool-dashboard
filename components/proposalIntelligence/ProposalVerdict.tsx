    ...
    <section className={cn(intelligenceSurfaceClasses.card, "mt-5")} aria-labelledby="verdict-title">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 flex-col items-start gap-3 lg:items-end">
          ...
          {stale ? (
            <>
              <RerunComparisonButton proposalId={proposalId} className="min-h-11 px-5 text-sm" />
              <Link href={`/proposals/${proposalId}/intelligence/comparisons/${workspace.run.runId}/evaluation#decision-record-title`} className="inline-flex min-h-9 items-center gap-1.5 text-xs font-bold text-brand-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"><ClipboardCheck size={14} aria-hidden="true" />Record a decision on this old result anyway</Link>
            </>
          ) : (
            <>
              <Link
