...
export default function ProposalVerdict({ workspace, proposalId }: { workspace: ComparisonWorkspace; proposalId: string }) {
  ...
  const verdict = recommendation.status === "no_eligible_vendor"
    ? "No vendor currently meets every must-pass requirement."
    ...
    <section className={cn(intelligenceSurfaceClasses.card, "mt-5")} aria-labelledby="verdict-title">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 flex-col items-start gap-3 lg:items-end">
          ...
          <Link
            href={`/proposals/${proposalId}/intelligence/comparisons/${workspace.run.runId}/evaluation#decision-record-title`}
            className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-brand-dark px-5 text-sm font-extrabold text-white shadow-sm transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2"
          >
            <ClipboardCheck size={16} aria-hidden="true" />Record your decision
          </Link>
          <p className="max-w-[13rem] text-xs leading-4 text-gray lg:text-right">Nothing is decided until you record it.</p>
        </div>
      </header>
      {stale && <div className={cn(intelligenceSurfaceClasses.block, "mt-5 bg-brand-muted")}><h3 className="text-sm font-extrabold text-navy">This result is out of date</h3><p className="mt-2 text-sm leading-6 text-gray">The proposal inputs changed after this comparison ran{workspace.freshness.reasons.length ? ` (${workspace.freshness.reasons.map(readable).join(" · ")})` : ""}. Run a new comparison before relying on it.</p></div>}
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        ...
        <article className={intelligenceSurfaceClasses.block}><h3 className="text-sm font-extrabold text-navy">Items to review before final decision</h3><ul className="mt-3 space-y-2">{recommendation.confidenceReasons.length ? recommendation.confidenceReasons.map((reason) => <li key={reason} className="rounded-xl bg-gray-panel p-3 text-sm text-gray">{readable(reason)}</li>) : <li className="rounded-xl bg-gray-panel p-3 text-sm text-gray">No additional recommendation review items are recorded.</li>}</ul></article>
