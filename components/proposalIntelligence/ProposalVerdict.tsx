  ...
  return (
    <section className={cn(intelligenceSurfaceClasses.card, "mt-5")} aria-labelledby="verdict-title">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><IntelligenceStatusChip status={stale ? "attention" : recommendation.status === "recommended" ? "complete" : "partial"} label={stale ? "Out of date" : recommendation.status === "close_call" ? "Close call" : readable(recommendation.status)} /></div>
      </header>
