...
export const createSourceProposalContextAction = async (proposalId:string,sourceIds:string[])=>await call<{jobId:string;runId:string;status:string;statusUrl:string;resultUrl:string}>(`/api/v1/proposals/${encodeURIComponent(proposalId)}/live-source-context-jobs`,{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({sourceIds})});
