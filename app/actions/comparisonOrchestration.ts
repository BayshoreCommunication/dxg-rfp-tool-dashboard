...
export type ComparisonWorkspace = ComparisonView & {
  ...
  intelligence: {
    ...
    evaluation: Array<{ participantId: string; vendorLabel: string; submittedScores: number; submittedEvaluators: number; weightedContributionTotal: number; evaluatorCount: number; completedEvaluatorCount: number; conflictCount: number; criteria?: Array<{ criterionId: string; name: string; meanScore: number; meanWeightedContribution: number; spread: number; rubricMaximum: number; originalWeight: number; /** How many of the scores behind meanScore RFPilot derived from evidence. 0/0 with humanCount means an older snapshot that never recorded origin. */ automatedCount: number; humanCount: number; /** The reason recorded with the score; a person's rationale when one exists, otherwise RFPilot's. */ rationale: string }> }>;
